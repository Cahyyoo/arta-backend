    ...
    <div class="max-w-5xl mx-auto px-4 mt-12">
      ...
      <section class="mb-10">
        <h2
          class="text-2xl font-black text-slate-800 mb-5 flex items-center gap-2"
        >
          3. Transaksi (Protected)
        </h2>

        <div
          class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mb-4"
        >
          <div
            class="p-4 border-b border-slate-100 bg-slate-50 flex items-center gap-4"
          >
            <span
              class="px-3 py-1 rounded-lg font-bold text-xs border method-get"
              >GET</span
            >
            <code class="text-base font-mono text-slate-700 font-bold"
              >/transactions</code
            >
          </div>
          <div class="p-5">
            <p class="text-slate-600 text-sm">
              Mengambil seluruh daftar transaksi khusus milik user yang sedang
              login.
            </p>
          </div>
        </div>
        ...
        >
          <div
            class="p-4 border-b border-slate-100 bg-slate-50 flex items-center gap-4 relative"
          >
            <span
              class="px-3 py-1 rounded-lg font-bold text-xs border method-post"
              >POST</span
            >
            <code class="text-base font-mono text-slate-700 font-bold"
              >/transactions</code
            >
            <span
              class="absolute right-4 top-1/2 -translate-y-1/2 text-xs bg-amber-100 text-amber-700 px-2 py-1 rounded-md font-bold"
              >Multipart/Form-Data</span
            >
          </div>
          <div class="p-5">
            <p class="mb-4 text-slate-600 text-sm">
              Menambah transaksi baru. Karena bisa mengirim file gambar
              (invoice), endpoint ini wajib menggunakan format
              <strong>form-data</strong>, bukan JSON.
            </p>
        ...
        <div
          class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mb-4 transition-all"
        >
          <div
            class="p-4 border-b border-slate-100 bg-amber-50/50 flex items-center gap-4 relative"
          >
            <span
              class="px-3 py-1 rounded-lg font-bold text-xs border method-put"
              >PUT</span
            >
            <code class="text-base font-mono text-slate-700 font-bold"
            ...
            >
            <span
              class="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] bg-amber-100 text-amber-700 px-2 py-1 rounded font-bold"
              >multipart/form-data</span
            >
          </div>
          <div class="p-5">
            <p class="mb-4 text-slate-600 text-sm">
              Memperbarui data transaksi yang sudah ada. Jika
              <code class="bg-slate-100 px-1">invoiceFile</code> dikirim,
              lampiran lama akan diganti.
            </p>
            <h4 class="font-bold text-[10px] text-slate-400 uppercase mb-2">
              Endpoint URL:
            </h4>
            <code class="text-sm text-indigo-600 bg-indigo-50 px-2 py-1 rounded"
              >/api/transactions/uuid-transaksi</code
            >
          </div>
        </div>
        ...
        >
          ...
          >
            <span
              class="px-3 py-1 rounded-lg font-bold text-xs border method-delete"
              >DELETE</span
            >
            <code class="text-base font-mono text-slate-700 font-bold"
              >/transactions/:id</code
          ...
          <div class="p-5">
            <p class="text-slate-600 text-sm">
              Menghapus transaksi berdasarkan ID. Backend otomatis mengecek agar
              user hanya bisa menghapus transaksinya sendiri.
            </p>
          </div>
        </div>
      </section>
      ...
      <section class="mb-10">
        ...
        >
          ...
          <div class="p-5">
            ...
            <h4
              class="font-bold text-xs uppercase tracking-wider text-slate-500 mb-2"
            >
              Request Body (JSON):
            </h4>
            <pre
              class="bg-slate-900 text-emerald-400 p-4 rounded-xl text-sm overflow-x-auto font-mono"
            >
{
